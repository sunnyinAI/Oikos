...
export default function AssistantPage() {
  ...
  useEffect(() => {
    let active = true;
    (getChatHistory() as Promise<Message[]>)
      .then((history) => { if (active) setMessages(history); })
      .catch(() => { if (active) addToast('Unable to load assistant history', 'error'); })
  ...
  const handleSend = async (preset?: string) => {
    ...
    try {
      ...
      setMessages((current) =>
        current.map((m) =>
          m.id === assistantId
            ? { ...m, content: msg.includes('ANTHROPIC_API_KEY') ? 'AI is not configured on the server yet. Add `ANTHROPIC_API_KEY` in `.env` to enable Ask Oikos.' : msg }
            : m,
        ),
      );
  ...
  const handleClear = async () => {
    ...
    } catch { addToast('Unable to clear chat history', 'error'); }
  };
    ...
    <div className="page-container flex h-full flex-col gap-4">
      <div className="rounded-2xl border border-saffron-200 bg-saffron-50 px-4 py-4 dark:border-saffron-900/40 dark:bg-saffron-950/20">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-saffron-900 dark:text-saffron-200">Ask Oikos</p>
          ...
          <Button variant="ghost" size="icon" onClick={handleClear}><RotateCcw size={16} /></Button>
        </div>
      </div>
      ...
      {!loadingHistory && messages.length === 0 && (
        <EmptyState icon="🤖" title="Start a chat with Oikos"
          ...
          {messages.map((message) => {
            ...
            return (
              <div key={message.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-3xl px-4 py-3 text-sm shadow-sm ${
                  isUser
