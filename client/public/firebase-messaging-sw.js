// Firebase Messaging Service Worker for Kharcha (Web Push / FCM).
...
async function init() {
  try {
    ...
    messaging.onBackgroundMessage((payload) => {
      ...
      const title = notification?.title || data?.title || 'Kharcha';
      const options = {
        body: notification?.body || data?.body || '',
        icon: '/kharcha-icon-192.png',
        badge: '/kharcha-icon-192.png',
        data: data || {},
      };
